...
function loadItemsData(): Set<number> {
  try {
    const itemsJsonPath = "./src/data/map/items.json";
    const itemsData: ItemData[] = JSON.parse(
      readFileSync(itemsJsonPath, "utf8"),
    );
    return new Set<number>(itemsData.map((item) => item.id));
  } catch (error) {
...
function validateAndFilterImages(
  imageFiles: string[],
  ...
  imageFiles.forEach((file) => {
    const itemId = basename(file, extname(file));
  ...
  try {
    ...
    return files
      .filter((file) => {
        ...
        return (
          statSync(filePath).isFile() && extname(file).toLowerCase() === ".png"
        );
      })
...
function generateSpritesheet(
  ...
  console.log(validItemIds.entries());
  ...
  imageFiles.forEach((file, index) => {
    const imageName = Number(basename(file, extname(file)));
    coordinates[index].item = imageName;
  });
...
function main() {
  ...
  try {
    ...
    console.error(
      "❌ ImageMagick not found! Please install ImageMagick first.",
    );
  ...
  configs.forEach((config, index) => {
    ...
    console.log(
      `📸 Converting ${config.spriteWidth}x${config.spriteHeight} spritesheet to WebP format...`,
    );
    ...
    try {
      ...
      console.log(
        "⚠️  PNG version is still available at:",
  ...
  configs.forEach((config) => {
    console.log(
      `   • ${config.spriteWidth}x${config.spriteHeight}px: ${config.outputImagePath.replace(".png", ".webp")}`,
    );
  });
}
