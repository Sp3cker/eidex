...
function FilterableDropdown({
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      setHighlighted((h) => (h <= 0 ? filteredOptions.length - 1 : h - 1));
      scrollToHighlighted(
  ...
  return (
    <div className="relative min-w-32 flex-1">
      ...
      {input && (
        <button
          type="button"
          className="absolute right-2 top-2 text-lg leading-none text-gray-400 hover:text-white"
          tabIndex={-1}
          onMouseDown={(e) => {
            e.preventDefault();
            setInput("");
      ...
      {open && (
        <ul
          ref={ulRef}
          id="dropdown-list"
          className="no-scrollbar absolute z-10 mt-1 max-h-48 w-full overflow-auto rounded-md border border-gray-700 bg-gray-800 shadow-lg ring-1 ring-black ring-opacity-5"
          role="listbox"
        >
          {filteredOptions.length > 0 ? (
            ...
            <li className="select-none px-3 py-1 text-sm text-gray-400">
              No results
            </li>
          ) : null}
        </ul>
  ...
  return (
    <div className="relative min-w-32 flex-1">
      <select
        ...
        className="h-9 w-full rounded-md border-0 bg-gray-800 pl-3 pr-8 text-sm text-white focus:ring-1 focus:ring-blue-400"
      >
...
function StatFilter({
  ...
  return (
    <div className="group flex h-9 flex-1 items-center justify-evenly rounded-md bg-gray-800 focus-within:ring-1 focus-within:ring-blue-400">
      <input
        type="number"
        placeholder="Min"
        value={minStat ?? ""}
        ...
        className="h-9 w-10 border-0 bg-transparent pl-2 text-sm text-white focus:outline-none focus:ring-0"
      />
