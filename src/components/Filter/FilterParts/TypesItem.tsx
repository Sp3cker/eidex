...
function TypeBadgeSimple({
  ...
  return (
    <div
      className={`w-17 flex cursor-pointer select-none items-center justify-center whitespace-nowrap rounded-full border-2 bg-transparent py-0.5`}
      style={isSelected ? typeStyle : deselectedTypeStyle}
      onClick={onClick}
    >
      <p
        title={typeName}
        className={`text-sm ${isSelected ? "font-bold" : ""} text-white`}
      >
        {typeName}
      </p>
    </div>
