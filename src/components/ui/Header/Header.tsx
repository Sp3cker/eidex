...
const Header = () => {
  ...
  const getLinkClassName = (href: string) => {
    ...
    if (href === "/map") {
      isActive = location === "/" || location === "/map" || location.startsWith("/map/");
    }
    
    const baseClasses = "pkmnem-face-shadow font-calamity rounded-sm px-5 py-1 text-sm cursor-pointer hover: transition-colors";
    ...
    <div className="content-visibility relative flex w-full items-center justify-between bg-gray-800 px-1 py-1">
      <div className="flex flex-row items-center gap-1">
        ...
        <div className="flex gap-1">
          <Link href="/dex">
            <button className={getLinkClassName("/dex")}>
              Dex
            </button>
          </Link>

          <Link href="/map">
            <button className={getLinkClassName("/map")}>
              Map
            </button>
          </Link>
        </div>
